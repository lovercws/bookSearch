...
public class Install {
	private static String CMS_PROPERTIES = "properties/jdbc.properties";
	...
	private void welcome() {
		...
		System.out.println("\n\n【重要】在开始前，您需要配置 " + CMS_PROPERTIES + "，此文件包含数据连接的相关信息。");
	}
	...
	private boolean importData() {
		...
		try {
			BufferedInputStream bis = new BufferedInputStream(new FileInputStream(CMS_PROPERTIES));
			Properties props = new Properties();
			props.load(bis);
			String url = props.getProperty("mysql.url");
			String driver = props.getProperty("mysql.driverClassName");
			...
			runner.runScript(new InputStreamReader(new FileInputStream(CMS_INSTALL_SQL), "UTF-8"));
			return true;
		} catch (Exception e) {
